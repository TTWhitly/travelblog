...
chatbotToggle.addEventListener("click", () => {
  chatbotWindow.classList.toggle("hidden"); // Toggle the "hidden" class
});
  ...
  fetch("http://127.0.0.1:5000/chat", {
    ...
    .then((data) => {
      if (data.message) {
        ...
        addMessageToChatBox(data.message, "ai");
      } else {
        addMessageToChatBox("Error: Could not get a response.", "ai");
      }
    })
    .catch((error) => {
      console.error("Error:", error);
      addMessageToChatBox("Error: Could not connect to the server.", "ai");
    });
});